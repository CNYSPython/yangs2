...
//    [user setObject:cookie.value forKey:@"JSESSIONID"];
//    [user setObject:self.phoneText.text forKey:@"phone"];
//    [user setObject:self.pwdText.text forKey:@"pwd"];
//    [user synchronize];
//
//}
...
//    NSString *timeSp = [[NSString stringWithFormat:@"%f", [datenow timeIntervalSince1970]] substringToIndex:10];
//    if (USER_ID && DEVICEID) {
//        [dic setObject:DEVICEID forKey:@"deviceid"];
//        [dic setObject:USER_ID forKey:@"user_id"];
//        [dic setObject:timeSp forKey:@"time_stamp"];
//    }else{
//        NSString *uuid = [NSUUID UUID].UUIDString;
//        [dic setObject:[[uuid MD5] MD5] forKey:@"deviceid"];
//        [dic setObject:timeSp forKey:@"time_stamp"];
//        
//    }
    return dic;
...
{
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic setObject:@"" forKey:@"JSESSIONID"];
    //获取当前时间戳
//    NSDate *datenow = [NSDate date];
//    NSString *timeSp = [[NSString stringWithFormat:@"%f", [datenow timeIntervalSince1970]] substringToIndex:10];
//    [dic setDictionary:@{@"currentDate":timeSp,
//                         @"isResponseJson":[NSNumber numberWithInt:1],
//                         @"loginType":@"App_IOS",
//                         @"sessionId":@""
//                         }];
    return dic;
...
+(double )LantitudeLongitudeDistance:(double )other_lon other_Lat:(double )other_lat self_Lon:(double )self_lon self_Lat:(double )self_lat{
    ...
    double radlong2 = PI*self_lon/180.0f;
...
+ (NSString *)htmlEntityDecode:(NSString *)str{
    ...
    str = [str stringByReplacingOccurrencesOfString:@"&lt;" withString:@"<"];
    str = [str stringByReplacingOccurrencesOfString:@"&gt;" withString:@">"];
    str = [str stringByReplacingOccurrencesOfString:@"&amp;" withString:@"&"]; // Do this last so that, e.g. @"&amp;lt;" goes to @"&lt;" not @"<"
    ...
    for (NSTextCheckingResult *item in result) {
        ...
        if (tmpArray.count >= 2) {
            ...
            if (loc != NSNotFound) {
                ...
                NSString *url = [src stringByRemovingPercentEncoding];
...
+ (BOOL)checkPassword:(NSString *) password{
    NSString *pattern = @"^(?=.*[0-9])(?=.*[a-zA-Z])(.{6,12})$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES%@",pattern];
...
+ (NSString *)timeDifference:(NSString *)starttime endTime:(NSString *)endtime {
    ...
    [sysFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    [sysFormatter setTimeZone:GTMzone];
    NSDate *sysTime = [sysFormatter dateFromString:endtime];
    NSDate *pastTime = [sysFormatter dateFromString:starttime];
    NSTimeInterval late1 = [pastTime timeIntervalSince1970]*1;
    NSTimeInterval late2 = [sysTime timeIntervalSince1970]*1;
    ...
    if ([hour intValue] > 0) {
        if ([hour intValue] > 24) {
            NSInteger tian = [hour integerValue] / 24;
            if (tian < 30) {
                timeString = [NSString stringWithFormat:@"%ld",tian*24*60*60];
            }
            else {
                timeString = @"0";
            }
            ...
            timeString = [NSString stringWithFormat:@"%ld",[hour integerValue]*60*60];
        }
    }
    else {
        if ([min intValue] > 0) {
            timeString = [NSString stringWithFormat:@"%ld",[min integerValue]*60];
        }
        else {
            if ([sen intValue] >= 0) {
                timeString = [NSString stringWithFormat:@"%ld",[sen integerValue]];
            }
        }
    }
    return timeString;
...
+ (BOOL)newString:(NSString *)newStr oldstr:(NSString *)str{
    //19世纪末20世纪初英美等国家出现了运用专业方法帮助有困难的群体解决其基本生存问题的职业活动，这就是社会工作。这里所说的“社会工作”是英文social work的翻译语。Social一词的基本含义是“社会的”或“社会性的

    if (newStr != nil) {
        if ([newStr rangeOfString:str].location == NSNotFound) {
            return NO;
...
+ (NSDate *)newTime:(NSString *)time formatter:(NSString *)dateFrom{
    ...
    [formatter setDateFormat:dateFrom];
    //NSString转NSDate
...
+ (NSString *)timeString:(NSString *)str zifu:(NSString *)zf{
    ...
    if ([self newString:str oldstr:zf]){
        NSArray *arr = [str componentsSeparatedByString:zf];
        if (arr.count > 2){
...
+ (NSString *)returnBankName:(NSString*) idCard{
    //"发卡行.卡种名称",
    NSArray* bankName = @[
                          @"邮储银行·绿卡通", @"邮储银行·绿卡银联标准卡", @"邮储银行·绿卡银联标准卡", @"邮储银行·绿卡专用卡", @"邮储银行·绿卡银联标准卡",@"邮储银行·绿卡(银联卡)", @"邮储银行·绿卡VIP卡", @"邮储银行·银联标准卡", @"邮储银行·中职学生资助卡", @"邮政储蓄银行·IC绿卡通VIP卡",
...
+ (void)accessCameraAlbum:(UIViewController *)viewController  Buttontype:(NSInteger )type block:(AccessCameraBlock)block{
    ...
    if (type == 0) {
        [typeArray addObjectsFromArray:@[@"拍照",@"从相册选择"]];
    }else if (type == 1){
        [typeArray addObjectsFromArray:@[@"拍照"]];
    }else {
        [typeArray addObjectsFromArray:@[@"拍照",@"查看"]];
    }
    [UIAlertController showActionSheetInViewController:viewController withTitle:nil message:nil cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles: typeArray popoverPresentationControllerBlock:nil tapBlock:^(UIAlertController * _Nonnull controller, UIAlertAction * _Nonnull action, NSInteger buttonIndex) {
        if (buttonIndex == 2){
            ...
            {
                // 无权限 引导去开启
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"相机授权未开启" message:@"请在系统设置中开启相机授权" preferredStyle:UIAlertControllerStyleAlert];
                [alertController addAction: [UIAlertAction actionWithTitle: @"暂不" style: UIAlertActionStyleDefault handler:nil]];
            ...
            {
                // 无权限 引导去开启
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"相册授权未开启" message:@"请在系统设置中开启相册授权" preferredStyle:UIAlertControllerStyleAlert];
                [alertController addAction: [UIAlertAction actionWithTitle: @"暂不" style: UIAlertActionStyleDefault handler:nil]];
                [alertController addAction: [UIAlertAction actionWithTitle: @"去设置" style: UIAlertActionStyleDefault handler:^(UIAlertAction *action){
                    NSURL *url = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
                    if ([[UIApplication sharedApplication]canOpenURL:url]) {
